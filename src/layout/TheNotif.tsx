...
const TheNotif = ({ notif, closeNotif }: Props) => {
  const pict = notif.avatar.includes('http')
    ? notif.avatar
    : PICT_URL + notif.avatar
  ...
  return (
    <motion.div
      className='alert-container the-alert-container'
    ...
    >
      <div className='container'>
        <div className='item'>
          <div
            className='avatar'
            style={{ backgroundImage: `url(${generateBGImgUrl(pict)})` }}
          ></div>
          <div className='content'>
            <div className='alert-info'>
              <span>{notif.nickname}</span> {notif.content}
            </div>
