...
const CompanyInfos = (props: Props) => {
  ...
  const formatDate = (timestamp: any) => {
    ...
    return `inscrit le ${year}-${month < 10 ? '0' + month : month}-${
      day < 10 ? '0' + day : day
    } a ${hour < 10 ? '0' + hour : hour}:${minute < 10 ? '0' + minute : minute}`
  }
  const exp = tot?.totalSoldeParticipationByCompany - tot?.totalPrice
  const str = exp.toString()

  const handleRestrict = async () => {
    try {
      const rest = await axios.patch(`${API_URL}/company/restrictCompany/${id}`)
          ...
          <div className='profileContent'>
            <div className='profileAvatar'>
              <div className='avatar'>
                <img
                  src={`${generateImgUrl(companyInfos?.logo ?? '')}`}
                  alt=''
              ...
              <div className='profileInfos'>
                ...
                <div className='profileRow profileGrid'>
                  ...
                  <div className='profileGridItem'>
                    ...
                    <div className='value'>
                      {companyInfos?.matricule_fiscale}
                    </div>
                  </div>
                </div>
            ...
            <div className='comp-prod-container'>
              {company &&
                company?.map((product: any) => (
                  <div key={product._id} className='participProductItem'>
